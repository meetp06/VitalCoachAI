...
export function register(server, ctx) {
    server.registerTool('query_behavior', {
        ...
        if (events.length === 0) {
            return { content: [{ type: 'text', text: 'No behavioral events found for this period.' }] };
        }
        const byCategory = {};
        for (const e of events) {
            ...
            if (e.dopamineDebtScore != null)
                entry.maxDebt = Math.max(entry.maxDebt, e.dopamineDebtScore);
        }
        ...
        for (const [cat, stats] of Object.entries(byCategory)) {
            ...
            if (stats.maxDebt > 0)
                lines.push(`- **Max Dopamine Debt**: ${stats.maxDebt.toFixed(0)}/100`);
            lines.push('');
        }
        return {
