...
export function register(server, ctx) {
    server.tool('query_causal_patterns', 'Get discovered causal patterns from the health graph. Patterns show relationships like \'high-GL meals \u2192 glucose spikes \u2192 poor sleep\'. Only returns patterns with 5+ observations.', {
    ...
    }, { readOnlyHint: true }, async ({ min_strength, limit }) => {
        ...
        if (patterns.length === 0) {
            return { content: [{ type: 'text', text: 'No significant causal patterns discovered yet. More data observations are needed.' }] };
        }
